...
function Test2Function(){
	//alert("I came here");
	var x = document.getElementById("myTestDiv").style.display;
	if (x === "block")
...
function myFormSubmitFunction(){
	$('#external_service').empty();
	var server = 'eventPictures/';
	var userid = document.getElementById("topNavHiddenPK").value;
	var friendid = document.getElementById("myInputField").value;
	$.ajax({
		url:'http://localhost:8080/facebook_v01/webapi/myTestResource/fetchGroupList/'+userid+'/'+friendid,
		type:'GET',
		async: false,
		dataType: 'json',
		success: function(data){
			...
			for( var i=0 ; i < no_of_object ;i++){
				...
				var groupdetails_id = data[i]['groupdetails_id'];
				var groupdetails_name = data[i]['groupdetails_name'];
			
				//alert (event_name + ", " + event_pk);
                ...
                '</span><a href="#" onclick="flist('+groupdetails_id+','+userid+','+friendid+')">'+groupdetails_name+'</a><div/>';
...
function flist(groupdetails_id,useridd,friendidd){
	$('#friendlist').empty();
	
	//alert("Hello I am here " + event_pk);
	var server = 'UserProfilePictures/';
	$.ajax({
			url:'http://localhost:8080/facebook_v01/webapi/testMemberResource/fetchGroupMemberList/'+groupdetails_id,
			type:'GET',
			dataType: 'json',
			async: false,
			success: function(data)
			{
				...
				for( var i=0 ; i < no_of_object ;i++){
					...
					var userdetails_picurl = server + data[i]['userdetails_picurl'];
					var userpkk = data[i]['userdetails_pk'];
					var popoverVariable;
					var admin = '';
					var c;
					
					$.ajax({
						url:'http://localhost:8080/facebook_v01/webapi/testMemberResource/fetchAdmin/'+groupdetails_id,
						type:'GET',
						dataType: 'json',
						async: false,
						success: function(data){
							
							var no_of_object2 = data.length;
							for( var j=0 ; j < no_of_object2 ;j++){
								
								admin = data[j]['userdetails_firstname'] + " " + data[j]['userdetails_lastname'];
					...
					$.ajax({
						url:'http://localhost:8080/facebook_v01/webapi/testMemberResource/fetchCount/'+useridd+'/'+friendidd+'/'+userpkk,
						type:'GET',
						dataType: 'json',
						async: false,
						success: function(data){
							
							var no_of_object3 = data.length;
							for( var k=0 ; k < no_of_object3 ;k++){
					...
					popoverVariable = '<a style="color:#3b5998" href="#" data-html="true" class="x" data-toggle="popover"  data-trigger="hover" data-placement="bottom" data-content=\'<div class="media"><a href="#" class="pull-left"></a><div class="media-body"><h4 class="media-heading"></strong></h4><p style="">Admin : '+admin+' count : '+c+'</p><p style="color:grey"></p><p style="color:grey"></p></div></div>\'>'+name+'</a>';
					
					var element = '<br/><div class="sidebar-name">'+ '<span class="pull-left" style="margin-right:12px; margin-top:-2px;">'+
            		'<img src="'+userdetails_picurl+'"  title="'+name+'" alt="John Doe" width="40px" height="30px">'+
